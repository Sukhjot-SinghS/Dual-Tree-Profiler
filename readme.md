...
A performance benchmarking and visualization tool for comparing AVL and Red-Black tree balancing strategies. Built with stateless C++ engines and a real-time React interface.

![10k Benchmark Results](docs/benchmark-results.png)
...
| **Left Rotations** | 3,561 | 2,934 | - |
| **Right Rotations** | 3,309 | 2,724 | - |
...
### 2. Step Through Balancing Operations
...
![Split-View Visualization](docs/split-view.png)
...
This design enables:
- **Instant undo/redo**: Rebuild tree state from history array
...
- **Undo**: Rewind operation history and rebuild tree state
- **Trash All**: Clear both trees and reset metrics
- **Rand (50)**: Stress test with 50 random insertions

### Visualization Features
- **Split View**: Compare AVL and RBT side-by-side during same operation
- **Single View**: Focus on one engine in fullscreen
- **Zoom/Pan**: Mouse wheel to zoom, click-drag to pan canvas
- **Frame Stepping**: `«` and `»` buttons to scrub through animation frames
...
### React DOM Reconciliation: The "Identity Theft" Trick
...
- **Left-Left** (balance > 1, left-heavy): Single right rotation
- **Right-Right** (balance < -1, right-heavy): Single left rotation  
- **Left-Right** (balance > 1, but left child is right-heavy): Left rotation on child, then right rotation on parent
- **Right-Left** (balance < -1, but right child is left-heavy): Right rotation on child, then left rotation on parent
...
6. **React SVG renderer**: Recursive tree drawing with zoom/pan transforms
...
**Built to understand the real cost of tree balancing strategies.** The numbers don't lie: on random insertion workloads, Red-Black trees win by avoiding structural rotations. AVL trees pay for their strict balance with measurably slower modification times.